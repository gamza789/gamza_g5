...
async function loadLoginJobs() {
    let jobs = [];
    const filePath = path.join(__dirname, 'url_login.txt'); // 파일 경로 지정
    ...
    try {
        ...
        if (e.code === 'ENOENT') {
            console.log("⚠️ url_login.txt 파일이 없어서 봇이 새로 생성했습니다!");
            await fs.writeFile(filePath, '', 'utf8');
        } else {
            console.log("⚠️ url_login.txt 파일이 비어있거나 읽을 수 없습니다.");
        }
    }
    return jobs;
}
